...
#mydata <- read.csv("//INSERTPATH/Carboniferous-vetted.csv", row.names = NULL, sep=",")

#from web
mydata <- read.csv(url("https://raw.githubusercontent.com/fossilrich/div-part/master/Carboniferous-vetted.csv"), row.names = NULL, sep=";")
...
CarboDat <- cbind(Age, AlphaForm, BetaWForm, BetaSForm, BetaJForm, GammaForm, CollpF, 
                Duration, RefForm, Formation, Environments)

write.csv(CarboDat, file = "05_CarboDat.csv")
