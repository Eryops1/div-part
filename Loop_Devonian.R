...
#mydata <- read.csv("//INSERTPATH/Devonian-vetted.csv", row.names = NULL, sep=",")

#from web
mydata <- read.csv(url("https://raw.githubusercontent.com/fossilrich/div-part/master/Devonian-vetted.csv"), row.names = NULL, sep=";")
...
DevoDat <- cbind(Age, AlphaForm, BetaWForm, BetaSForm, BetaJForm, GammaForm, CollpF, 
                Duration, RefForm, Formation, Environments)

write.csv(DevoDat, file = "04_DevoDat.csv")
