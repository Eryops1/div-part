...
#mydata <- read.csv("//INSERTPATH/Paleogene-vetted.csv", row.names = NULL, sep=",")

#from web
mydata <- read.csv(url("https://raw.githubusercontent.com/fossilrich/div-part/master/Paleogene-vetted.csv"), row.names = NULL, sep=";")
...
PaleogDat <- cbind(Age, AlphaForm, BetaWForm, BetaSForm, BetaJForm, GammaForm, CollpF, 
                Duration, RefForm, Formation, Environments)

write.csv(PaleogDat, file = "015_PaleogDat.csv")
